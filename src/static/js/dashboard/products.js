const products = (categories, order) => {
  fetch("/api/shelf/products/").then((response) => {
        if (!response.ok) {
        ...
        results.forEach((product) => {
          const categories = results.categories.map(category => category[0]);
          const files = results.files.map(file => file.file);
          ...
          if (window.role === "admin"){
            cost += `<li class="flex items-center gap-2">
                ...
                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">${product.cost_price}</p>
              </li>`;}
          parseFloat(product.cost_price).toFixed(2);

          /* Adding images to the product */
          images = `
            <div id="${product.id}-controls-gallery" class="relative w-full" data-carousel="slide">
              <!-- Carousel wrapper -->
          ...
          html += `
            ...
            <div class="pt-6">
              <div class="mb-4 flex items-center justify-between gap-4">
                <span class="me-2 rounded bg-primary-100 px-2.5 py-0.5 text-xs font-medium text-primary-800 dark:bg-primary-900 dark:text-primary-300"> ${category[0]} </span>
              ...
              <ul class="mt-2 flex items-center gap-4">
                <li class="flex items-center gap-2">
                  ...
                  <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Piece-${recorder_quantity}</p>
                </li>
                ${cost}
              </ul>

              <div class="mt-4 flex items-center justify-between gap-4">
                <p class="text-2xl font-extrabold leading-tight text-gray-900 dark:text-white">NGN ${product.selling_price}</p>
...
const categories = () => {
  fetch("/api/shelf/categories/").then((response) => {
        if (!response.ok) {
...
const load_products = () => {
  ...
  if (response  && response != "") {
    ({ html, count, next, previous, results } = response);
  }

  product_section = document.getElementById("product-section");
