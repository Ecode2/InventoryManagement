...
const toggleModal = (modalId) => {
    const modal = document.getElementById(modalId);
        ...
        const closeModalOnClickOutside = function (event) {
            if (!modal.contains(event.target) && !event.target.matches(`#${modalId}, #${modalId} *`)) {
                modal.classList.add('hidden');
                window.removeEventListener('click', closeModalOnClickOutside);
            }
        };

        // Use setTimeout to allow the event stack to clear before adding the click listener
        setTimeout(() => {
