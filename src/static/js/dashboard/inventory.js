    ...
    return fetch(page)
        ...
        .then((data) => {
            ...
            results.forEach((inventory) => {
                ...
                image = ``;
                if (files) {
                    image += `<img src="${files[0]}" alt="${inventory.product.name}-Image" class="h-8 w-auto mr-3">`
                }
                let category = ``;
                if (categories) {
                    category += `<span class="bg-primary-100 text-primary-800 text-xs font-medium px-2 py-0.5 rounded dark:bg-primary-900 dark:text-primary-300">${categories[0]}</span>`;
                }
                ...
                if (role === "admin") {
                    ...
                    edit_btn += `
                        <li>
                            <button type="button" data-drawer-target="drawer-update-product-${inventory.id}" data-drawer-show="drawer-update-product-${inventory.id}" aria-controls="drawer-update-product-${inventory.id}" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                    ...
                    delete_btn += `
                        <li>
                            <button type="button" data-modal-target="delete-modal-${inventory.id}" data-modal-toggle="delete-modal-${inventory.id}" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 text-red-500 dark:hover:text-red-400">
                ...
                html += `
                    <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                        ...
                        <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <div class="flex items-center">
                                <div class="h-4 w-4 rounded-full inline-block mr-2 bg-${alert_color}-700"></div>
                                ${inventory.stock}
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-3 flex items-center justify-center">
                            <!-- Dropdown Button -->
                            <button id="${inventory.id}-${inventory.product.id}-dropdown-button" data-dropdown-toggle="${inventory.id}-${inventory.product.id}-dropdown" class="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                            ...
                            <div id="${inventory.id}-${inventory.product.id}-dropdown" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="${inventory.id}-${inventory.product.id}-dropdown-button">
                                    ${edit_btn}
                                    <li>
                                        <button type="button" data-drawer-target="drawer-read-product-${inventory.id}-advanced" data-drawer-show="drawer-read-product-${inventory.id}-advanced" aria-controls="drawer-read-product-${inventory.id}-advanced" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                            <svg class="w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                ...
                update_modal = `
                    <form action="#" id="drawer-update-product-${inventory.id}" class="fixed top-0 left-0 z-40 w-full h-screen max-w-3xl p-4 overflow-y-auto transition-transform -translate-x-full bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-update-product-${inventory.id}-label" aria-hidden="true">
                ...
                preview_modal = `
                    <!-- Preview Drawer -->
                    <div id="drawer-read-product-${inventory.id}-advanced" class="overflow-y-auto fixed top-0 left-0 z-40 p-4 w-full max-w-lg h-screen bg-white transition-transform -translate-x-full dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-label" aria-hidden="true">
                        <div>
